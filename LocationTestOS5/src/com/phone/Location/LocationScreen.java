...
public final class LocationScreen extends MainScreen
{
    ...
    public int ConnectionResponse=0;
    ...
    public RichTextField servertext;
    
    public LocationScreen()
    {
    	super(MainScreen.VERTICAL_SCROLLBAR | MainScreen.HORIZONTAL_SCROLLBAR);

    	//text = new RichTextField("Waiting for Co-ordinates...");
    	//servertext = new RichTextField("");
    	bbLoc = new BlackBerryLocationHandler();			//firing and initializing
    	
    	timer = new Timer();
    	timer.schedule(new Updater(), 1000, 10000);		//once in 10second
    	...
    	public void run() 
		{
    		...
    		if(latitude !=0.0 & longitude !=0.0)
    		{
    			synchronized(LocationApp.getEventLock()) //Locks the screen thread 
    													//IMPORTANT- can only be synchronized
    			{
    				accuracy = bbLoc.getAccuracy();
    				speed = bbLoc.getSpeed();
    				satellite_count = Integer.parseInt(bbLoc.getNumbSat());
    				...
    				bbLoc.SendtoServer();
    				//ConnectionResponse = bbLoc.getPrompt();
    			}
    		}
    		else
    		{
    			synchronized(LocationApp.getEventLock())
    			{
			    	...
			    	/**IMPORTANT - causing the app to work on GUI rather using 
			    	 	  		BlackBerryLocationHandler's Alternate Entry Point
			    	 	  		**/
